...
export class ExprContext {
  constructor(
    ...
    this.ignoreAttributesWithoutValue =
      opt_ignoreAttributesWithoutValue || false;
...
export class LocationExpr {
  ...
  appendStep(s) {
    const combinedStep = this._combineSteps(
...
export class StepExpr {
  ...
  evaluate(ctx) {
    ...
    if (this.axis == xpathAxis.ANCESTOR_OR_SELF) {
      ...
      let tagName = xpathExtractTagNameFromNodeTest(
        this.nodetest,
...
let xpathfunctions = {
  ...
  "local-name"(ctx) {
    ...
    if (n.length == 0) {
      ...
      let value = new StringValue(n[0].nodeName);
      //console.log(value);
      return value;
    }
  },
  ...
  evaluate(ctx) {
    ...
    for (let i = 0; i < this.predicate.length; ++i) {
      ...
      for (let j = 0; j < nodes0.length; ++j) {
        const n = nodes0[j];
        if (
          this.predicate[i].evaluate(ctx.clone(n, j, nodes0)).booleanValue()
        ) {
          nodes.push(n);
        }
      }
